...
---
### Calculation
Points on a sphere with radius r are described in 3 dim euclidean space as
p=(x,y,z)=( r.cos( \phi ).cos( \lambda ), r.cos( \phi ).sin( \lambda ), r.sin( \phi ) )
where \phi is the latitude and \lambda is the longitude. The cord between two points a and b is described by p(t)=(b-a)/c * t + a for 0<=t<=1 where c is the time interval
